...
__global__ void glensing(const float *lens_x, const float *lens_y, const float *lens_mass, const size_t nobjects, unsigned int* results, const d_constants* v, int threadNo) {
	...
	float start_x, start_y, dx, dy, noise_x, noise_y;
	size_t k, it;
	
	for(it = 0; it < v->rpp; ++it) {
		if(!SUDO_RAND) {
			//__device__ __shared__ curandStateSobol32_t sharedCurandState;
			//noise_x = curand_uniform(&sharedCurandState);
			//noise_y = curand_uniform(&sharedCurandState);
			//start_x = initial_x + noise_x * increment_x;
			//start_y = initial_y + noise_y * increment_y;
		}	
		if(SUDO_RAND){
			noise_x = (threadNo%4)/4;
			noise_y = threadNo/16;
			start_x = initial_x + noise_x * increment_x;
			start_y = initial_y + noise_y * increment_y;
		}
